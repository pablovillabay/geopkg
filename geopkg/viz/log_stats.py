...
def cat_stats(logs, label, filter, cpal, ylim=100):
    ...
    if len(filter) == 1:    
        ...
        df3.sort_values(by=[filter[1], filter[0]], ascending=(True, True), inplace=True)
        g = sns.FacetGrid(df3, row = filter[0], col=filter[1], height=3.5, margin_titles=True, 
                            gridspec_kws={'wspace': 0.03}, subplot_kws={'ylim':[0,ylim]}, 
                            row_order= np.sort(list(df3[filter[0]].unique())), 
                            col_order= np.sort(list(df3[filter[1]].unique())))
    ...
    if cat != None:
        for idx_cat in range(len(z_categories)):
            ...
            if shade > 0.0:
                ...
                sns.kdeplot(df2[df2[cat]==z_cat][x_], df2[df2[cat]==z_cat][y_], cmap=newcmp, shade=True, 
...
def summary_boxplot(logs, wells, cat, props, cpal, sr=0.5):
    ...
    fig, ax = plt.subplots(len(df1.columns)-2, 1, sharex=True, figsize=(15,12))
